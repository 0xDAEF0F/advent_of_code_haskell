module SupplyStacksA (solution) where
...
executeInstruction cr (qty, from, to) = executeInstruction newStacks (qty - 1, from, to)
  where
    crateToMove = head (cr !! from)
    newFromStack = tail (cr !! from)
    newToStack = crateToMove : (cr !! to)
